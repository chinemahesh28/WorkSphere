...
const TrainerHome = ({ setActivePage }) => {
  ...
  const loadData = () => {
    ...
    const classUpdates = JSON.parse(localStorage.getItem('classUpdates')) || [];
    const materials = JSON.parse(localStorage.getItem('trainerMaterials')) || [];

    // My batch IDs
    const myBatchIds = batchTrainerAssignments.filter(a => a.trainerId === trainerId).map(a => a.batchId);
    const myBatches = batches.filter(b => myBatchIds.includes(b.id));

    // Students in my batches
    const myStudentIds = new Set(
      studentBatchAssignments.filter(a => myBatchIds.includes(a.batchId)).map(a => a.studentId)
    );

    // My class updates & materials
    const myUpdates = classUpdates.filter(u => u.trainerId === trainerId);
    const myMaterials = materials.filter(m => m.trainerId === trainerId);
    ...
    const enriched = myBatches.map(batch => {
      const studentCount = studentBatchAssignments.filter(a => a.batchId === batch.id).length;
      const classCount = myUpdates.filter(u => u.batchId === batch.id).length;
    ...
    setRecentUpdates(myUpdates.slice(-5).reverse().map(u => ({
      ...u,
      batchName: batches.find(b => b.id === u.batchId)?.batchName || 'Unknown'
    })));
  };
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button onClick={() => setActivePage('class-update')} className="flex items-center gap-3 p-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all">
          ...
          <div className="text-left"><p className="font-bold">Log Class Update</p><p className="text-sm text-indigo-100">Record a session</p></div>
        </button>
        <button onClick={() => setActivePage('manage-records')} className="flex items-center gap-3 p-4 bg-white border border-gray-200 text-gray-800 rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
          <svg className="w-8 h-8 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>
          <div className="text-left"><p className="font-bold">Manage Records</p><p className="text-sm text-gray-500">View class history</p></div>
        </button>
        <button onClick={() => setActivePage('upload-materials')} className="flex items-center gap-3 p-4 bg-white border border-gray-200 text-gray-800 rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
          ...
          <div className="text-left"><p className="font-bold">Upload Materials</p><p className="text-sm text-gray-500">Share PDFs & resources</p></div>
        </button>
      </div>
