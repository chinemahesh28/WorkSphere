...
const ClassUpdate = ({ setActivePage }) => {
  ...
  const saveUpdate = async (fileData, fileName) => {
    const newUpdate = {
      trainerId,
    ...
    try {
      await api.post('/api/class-updates', newUpdate);
      ...
      toast.error(err.response?.data?.message || 'Save failed — error contacting server.');
    }
