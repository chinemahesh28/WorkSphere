...
const ManageRecords = ({ setActivePage }) => {
  ...
  const loadData = async () => {
    try {
      const [batchesRes, assignmentsRes, updatesRes] = await Promise.all([
        api.get('/api/batches'),
      ...
      const myBatchIds = (assignmentsRes.data || []).map(a => a.batchId || a.batch?.id);
      setBatches(allBatches.filter(b => myBatchIds.includes(b.id)));
      
      const myRecords = (updatesRes.data || []).map(u => ({
        ...u, batchName: allBatches.find(b => b.id === (u.batchId || u.batch?.id))?.batchName || 'Unknown'
      }));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/class-updates/${id}`);
      loadData();
    } catch (error) {
    ...
    const doSave = async (fileData, fileName) => {
      ...
      if (fileData !== undefined) {
        if (fileData) {
          ...
          updateData.materialTitle = null;
        }
      } else {
        updateData.fileData = editingRecord.fileData;
        updateData.fileName = editingRecord.fileName;
      }

      try {
        await api.put(`/api/class-updates/${editingRecord.id}`, updateData);
        ...
        toast.error('Save failed — attached file may be too large for storage or server error.');
      }
    };
