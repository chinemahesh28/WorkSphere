...
const CounsellorNavbar = ({ activePage, setActivePage }) => {
  ...
  useEffect(() => {
    ...
    if (currentUser.fullName) {
      setCounsellorName(currentUser.fullName);
    } else if (currentUser.username) {
      setCounsellorName(currentUser.username);
    }
    if (currentUser.email) {
      setCounsellorEmail(currentUser.email);
    }
  }, []);
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border border-gray-100 py-1 z-50">
                    <div className="px-4 py-3 border-b border-gray-100">
                      ...
                      <p className="text-xs text-gray-500">{counsellorEmail}</p>
                    </div>
