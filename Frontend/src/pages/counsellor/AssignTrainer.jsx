...
const AssignTrainer = () => {
  ...
  const loadData = async () => {
    try {
      const [batchesRes, trainersRes, assignmentsRes] = await Promise.all([
        api.get('/api/batches'),
        api.get('/api/users/role/TRAINER'),
        api.get('/api/batch-trainer-assignments')
      ]);
  ...
  const handleAssign = async () => {
    ...
    try {
      const response = await api.post('/api/batch-trainer-assignments', {
  ...
  const handleRemove = async (assignmentId) => {
    if (await toast.confirm('Remove this trainer assignment?', 'Remove Assignment')) {
      try {
        await api.delete(`/api/batch-trainer-assignments/${assignmentId}`);
  ...
  const handleSaveEdit = async (e) => {
    e.preventDefault();
    try {
      const response = await api.put(`/api/batch-trainer-assignments/${editingAssignment.id}`, {
        batchId: editingAssignment.batchId,
        trainerId: editingAssignment.trainerId
      });
