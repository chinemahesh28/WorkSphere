...
import { validateUserForm, hasErrors, getFieldClass } from '../../utils/validation';
...
const ManageCounsellors = () => {
  ...
  const loadCounsellors = async () => {
    setLoading(true);
    try {
      const response = await api.get('/api/users/role/COUNSELLOR');
  ...
  const handleAddCounsellor = async (e) => {
    ...
    try {
      const response = await api.post('/api/users', { ...formData, role: 'COUNSELLOR' });
      setCounsellors([...counsellors, response.data]);
      setShowAddModal(false);
      resetForm();
      toast.success('Counsellor added successfully!');
    } catch (error) {
      if (error.response?.data?.errors) {
        setFormErrors(prev => ({ ...prev, ...error.response.data.errors }));
      }
  ...
  // Update counsellor
  const handleUpdateCounsellor = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await api.put(`/api/users/${selectedCounsellor.id}`, { ...formData, role: 'COUNSELLOR' });
      setCounsellors(counsellors.map(c => c.id === selectedCounsellor.id ? response.data : c));
      ...
      toast.error(error.response?.data?.message || 'Failed to update counsellor');
    }
  };

  // Delete counsellor
  const handleDeleteCounsellor = async () => {
    try {
      await api.delete(`/api/users/${selectedCounsellor.id}`);
      setCounsellors(counsellors.filter(c => c.id !== selectedCounsellor.id));
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={getFieldClass(formErrors.username)}
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={getFieldClass(formErrors.password)}
                    minLength="6"
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.age}
                    onChange={handleInputChange}
                    className={getFieldClass(formErrors.age)}
                    min="18"
                    max="60"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={getFieldClass(formErrors.salary)}
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={getFieldClass(formErrors.phoneNo)}
                    required
                  />
                  {formErrors.phoneNo && <p className="mt-1 text-sm text-red-500">{formErrors.phoneNo}</p>}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={getFieldClass(formErrors.experience)}
                    min="0"
                    max="50"
                  />
