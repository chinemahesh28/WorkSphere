...
import { validateUserForm, hasErrors, getFieldClass, validateRequired } from '../../utils/validation';
...
const ManageTrainers = () => {
  ...
  const handleAddTrainer = async (e) => {
    ...
    errors.domain = validateRequired(formData.domain, 'Domain');
    setFormErrors(errors);
    if (hasErrors(errors)) return;

    try {
      const response = await api.post('/api/users', { ...formData, role: 'TRAINER' });
  ...
  const handleUpdateTrainer = async (e) => {
    ...
    try {
      const response = await api.put(`/api/users/${selectedTrainer.id}`, { ...formData, role: 'TRAINER' });
      setTrainers(trainers.map(t => t.id === selectedTrainer.id ? response.data : t));
  ...
  const handleDeleteTrainer = async () => {
    try {
      await api.delete(`/api/users/${selectedTrainer.id}`);
