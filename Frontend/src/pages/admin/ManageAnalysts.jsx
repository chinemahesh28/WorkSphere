...
const ManageAnalysts = () => {
  ...
  const loadAnalysts = async () => {
    setLoading(true);
    try {
      const response = await api.get('/api/users/role/ANALYST');
  ...
  const handleAddAnalyst = async (e) => {
    ...
    try {
      const response = await api.post('/api/users', { ...formData, role: 'ANALYST' });
      setAnalysts([...analysts, response.data]);
      ...
      toast.error(error.response?.data?.message || 'Failed to add analyst');
    }
  };
  ...
  // Update analyst
  const handleUpdateAnalyst = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await api.put(`/api/users/${selectedAnalyst.id}`, { ...formData, role: 'ANALYST' });
      setAnalysts(analysts.map(a => a.id === selectedAnalyst.id ? response.data : a));
      ...
      toast.error(error.response?.data?.message || 'Failed to update analyst');
    }
  };
  ...
  const handleDeleteAnalyst = async () => {
    try {
      await api.delete(`/api/users/${selectedAnalyst.id}`);
